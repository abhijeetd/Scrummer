...
namespace Scrummer.Application
{
    public class StandupService
    {
        ...
        {
            return _actionItemRepository.GetFiltered(p => p.Date > context.Date.Date && p.Date < context.Date.AddDays(1).Date);
        }
        ...
        {
            var list = _userRepository.GetFiltered(p => p.Project == context.Project).Select(p => new IndividualStatus
            {
                AssignedTo = p.Username
            }).ToList();
